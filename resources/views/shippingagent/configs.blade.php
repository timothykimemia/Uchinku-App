	...
	<div class="col-sm-12">
		...
		<div class="panel">
			<div class="panel-heading">
				<h3 class="panel-title text-center">{{__('Shipping Agents Registration Form')}}</h3>
			</div>
			<div class="panel-body">
				<form action="{{ route('shippingagent.configs.store') }}" method="post" enctype="multipart/form-data">
					@csrf
					<div class="row">
						<div class="col-lg-8 form-horizontal" id="form">
							@foreach (json_decode(\App\ShippingagentConfig::where('type', 'verification_form')->first()->value) as $key => $element)
		...
		function appenddToForm(type){
			...
			if(type == 'text'){
				...
				var str = '<div class="form-group" style="background:rgba(0,0,0,0.1);padding:10px 0;">'
								...
								+'</div>'
								+'<div class="col-lg-7">'
									+'<input class="form-control" type="date" name="label[]" placeholder="Label">'
								+'</div>'
								+'<div class="col-lg-2">'
									+'<span class="btn btn-icon btn-circle icon-lg fa fa-time" onclick="delete_choice_clearfix(this)"></span>'
								+'</div>'
							+'</div>';
				$('#form').append(str);
			}
