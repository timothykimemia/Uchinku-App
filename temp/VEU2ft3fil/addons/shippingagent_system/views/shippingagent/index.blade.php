...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="panel">
                <div class="panel-heading">
                    <h3 class="panel-title text-center">{{__('Basic Shippingagent')}}</h3>
                </div>
                <div class="panel-body">
                    <form class="form-horizontal" action="{{ route('shippingagent.store') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            ...
                            <div class="col-lg-6">
                                @php
                                    if(\App\ShippingagentOption::where('type', 'user_registration_first_purchase')->first() != null){
                                        $percentage = \App\ShippingagentOption::where('type', 'user_registration_first_purchase')->first()->percentage;
                                        $status = \App\ShippingagentOption::where('type', 'user_registration_first_purchase')->first()->status;
                                    }
                                    else {
                                        $percentage = null;
                                    }
        ...
        <div class="col-lg-6">
            <div class="panel">
                <div class="panel-heading">
                    <h3 class="panel-title text-center">{{__('Product Sharing Shippingagent')}}</h3>
                </div>
                <div class="panel-body">
                    <form class="form-horizontal" action="{{ route('shippingagent.store') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            ...
                            <div class="col-lg-6">
                                @php
                                    if(\App\ShippingagentOption::where('type', 'product_sharing')->first() != null && \App\ShippingagentOption::where('type', 'product_sharing')->first()->details != null){
                                        $commission_product_sharing = json_decode(\App\ShippingagentOption::where('type', 'product_sharing')->first()->details)->commission;
                                        $commission_type_product_sharing = json_decode(\App\ShippingagentOption::where('type', 'product_sharing')->first()->details)->commission_type;
                                        $status = \App\ShippingagentOption::where('type', 'product_sharing')->first()->status;
                                    }
                                    else {
    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="panel">
                <div class="panel-heading">
                    <h3 class="panel-title text-center">{{__('Product Sharing Shippingagent (Category Wise)')}}</h3>
                </div>
                <div class="panel-body">
                    <form class="form-horizontal" action="{{ route('shippingagent.store') }}" method="POST">
                        @csrf
                        @php
                            if(\App\ShippingagentOption::where('type', 'category_wise_shippingagent')->first() != null){
                                $category_wise_shippingagent_status = \App\ShippingagentOption::where('type', 'category_wise_shippingagent')->first()->status;
                            }
                        @endphp
                        <div class="form-group">
                            <div class="col-lg-4">
                            ...
                            <div class="col-lg-8">
                                <label class="switch">
                                    <input value="1" name="status" type="checkbox" @if ($category_wise_shippingagent_status)
                                        checked
                                    @endif>
                                    <span class="slider round"></span>
                                </label>
                        ...
                        @if (\App\ShippingagentOption::where('type', 'category_wise_shippingagent')->first() != null)
                            <input type="hidden" name="type" value="category_wise_shippingagent">
                            @foreach (\App\Category::all() as $key => $category)
                                @php
                                    $found = false;
                                @endphp
                                @if(\App\ShippingagentOption::where('type', 'category_wise_shippingagent')->first()->details != null)
                                    @foreach (json_decode(\App\ShippingagentOption::where('type', 'category_wise_shippingagent')->first()->details) as $key => $data)
